...
function canHorseMoveToBlock(selectedPiece, clickedBlock) {
    let canMoveLeft = clickedBlock.col === selectedPiece.col - 2 &&
        clickedBlock.row === selectedPiece.row + 1;
    let canMoveLeftFDown = clickedBlock.col === selectedPiece.col - 1 &&
        clickedBlock.row === selectedPiece.row + 2;
    let canMoveRightDown = clickedBlock.col === selectedPiece.col +1 &&
        clickedBlock.row === selectedPiece.row + 2;
    let canMoveLeftUp = clickedBlock.col === selectedPiece.col -2 &&
        clickedBlock.row === selectedPiece.row -1;
    let canMoveLeftUpUp = clickedBlock.col === selectedPiece.col -1 &&
        clickedBlock.row === selectedPiece.row -2;
    let canMoveRightUp = clickedBlock.col === selectedPiece.col +2 &&
        clickedBlock.row === selectedPiece.row -1;
    let canMoveRightUpUp = clickedBlock.col === selectedPiece.col +1 &&
        clickedBlock.row === selectedPiece.row -2;
    return (canMoveLeft || canMoveLeftFDown || canMoveRightDown
        || canMoveLeftUp || canMoveLeftUpUp || canMoveRightUp
        || canMoveRightUpUp)
}

function noFigureBetween(selectedPiece,clickedBlock) {
    for (let i = selectedPiece.col; i > clickedBlock.col; i--) {
        for (let j = 0; j < allFigures.white.length; j++) {
            if(allFigures.white[i].row === selectedPiece.row &&
                allFigures.white[i].col > clickedBlock.col &&
                allFigures.white[i].col < selectedPiece.col){
                return false;
            }
        }
    }
    for (let i = selectedPiece.col; i > clickedBlock.col; i--) {
        for (let j = 0; j < allFigures.black.length; j++) {
            if(allFigures.black[i].row === selectedPiece.row &&
                allFigures.black[i].col > clickedBlock.col &&
                allFigures.black[i].col < selectedPiece.col){
                return false;
            }
        }
    }
...
function noFigureBetweenRow(selectedPiece, clickedBlock) {
    for (let i = selectedPiece.row; i > clickedBlock.row; i--) {
        for (let j = 0; j < allFigures.white.length; j++) {
            if(allFigures.white[i].col === selectedPiece.col &&
                allFigures.white[i].row < clickedBlock.row &&
                allFigures.white[i].row < selectedPiece.row){
                return false;
            }
        }
    }
    for (let i = selectedPiece.row; i > clickedBlock.row; i--) {
        for (let j = 0; j < allFigures.black.length; j++) {
            if(allFigures.black[i].col === selectedPiece.col &&
                allFigures.black[i].row < clickedBlock.row &&
                allFigures.black[i].row < selectedPiece.row){
                return false;
            }
        }
    }
...
function canCastleMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
    let canMoveLeft = selectedPiece.row === clickedBlock.row &&
        noFigureBetween(selectedPiece,clickedBlock);
    let canMoveRight = selectedPiece.row === clickedBlock.row &&
        noFigureBetween(selectedPiece,clickedBlock);
    let canMoveUp = selectedPiece.col === clickedBlock.col &&
        noFigureBetweenRow(selectedPiece,clickedBlock);
    let canMoveDown = selectedPiece.col === clickedBlock.col &&
        noFigureBetweenRow(selectedPiece,clickedBlock);
